<?php
 include 'head.php';
?>
<style>


    h3 {
    font-size: 25px !important;
    margin-top: 20px;
    margin-bottom: 10px;
    line-height: 1.4em !important;
}

p {
    font-size: 14px;
    margin: 0 0 10px !important;
    font-weight: 300;
}

.container-fluid {
    padding-right: 15px!important;
    padding-left: 40px;
}

.card-header {
    background-color:#001055!important;
    color:#fff!important;
    font-size:1.6vmax;
}

</style>
	
	<?php
include 'sidebar/quicklinks.php';
?>

                <div class="col-12 col-md-9 mt-3 mt-sm-0 pt-5 ">
						<!-- Write content for right side here  -->

                        <div class="row mx-0" >
						    <h2 class=" text-center" style="color:#001055; text-transform: capitalize;
                             font-weight:700!important; font-size:2.4vmax"> Sports calendar<span class="d-block d-sm-inline"> for Session 2024-25</span></h2>


<div class="row col-mb-50">
    <div class="col-lg-12 col-md-12 col-sm-12 mt-0">
        <div class="card">
            <div class="card-header text-center fw-bold ">Semester System</div>
                <div class="card-body">
<div class="table-responsive">
<table class="table table-bordered table-striped">
  <tr>
    <th class="fw-semi-bold">S.No.</th>
    <th class="fw-semi-bold">College/Department</th>
    <th class="fw-semi-bold">College/Department wise sports schedule (Trails Only)</th>
  </tr>
  <tr>
    <td>1</td>
    <td>TMIMT College of Physical Education</td>
    <td>18-19 September, 2023</td>
  </tr>
  <tr>
    <td>2</td>
    <td>College of Fine Arts</td>
    <td>20-21 September, 2023</td>
  </tr>
  <tr>
    <td>3</td>
    <td>Faculty of Education</td>
    <td>22-23 September, 2023</td>
  </tr>
  <tr>
    <td>4</td>
    <td>College of Law & Legal Studies</td>
    <td>25-26 September, 2023</td>
  </tr>
  <tr>
    <td>5</td>
    <td>Faculty of Engineering</td>
    <td>1-3 November, 2023</td>
  </tr>
  <tr>
    <td>6</td>
    <td>Department of Physiotherapy</td>
    <td>27-28 September, 2023</td>
  </tr>
  <tr>
    <td>7</td>
    <td>Medical College & Research Centre</td>
    <td>9-11 October, 2023</td>
  </tr>
  <tr>
    <td>8</td>
    <td>College of Management(TMIMT)</td>
    <td>3-5 October, 2023</td>
  </tr>
  <tr>
    <td>9</td>
    <td>College of Pharmacy</td>
    <td>29-30 September, 2023</td>
  </tr>
  <tr>
    <td>10</td>
    <td>Dental College & Research Centre</td>
    <td>12-14 October, 2023</td>
  </tr>
  <tr>
    <td>11</td>
    <td>College of Paramedical Sciences</td>
    <td>25-27 October, 2023</td>
  </tr>
  <tr>
    <td>12</td>
    <td>College of Computing Sciences & Information Technology</td>
    <td>8-30 November, 2023</td>
  </tr>
  <tr>
    <td>13</td>
    <td>College of Agriculture Sciences</td>
    <td>7-9 December, 2023</td>
  </tr>
  <tr>
    <td>14</td>
    <td>College of Nursing</td>
    <td>4-6 December, 2023</td>
  </tr>
  <tr>
    <td>16</td>
    <td>Last Date of Entry for Intercollegiate (University Level)</td>
    <td>20 February, 2024</td>
  </tr>
  <tr>
    <td>17</td>
    <td>Intercollegiate (University Level)</td>
    <td>26 February-20 March, 2024</td>
  </tr>
</table>
</div>


						</div>
					</div>
				</div>
			</div>
		</section><!-- #content end -->


        
<?php
 include 'footer.php';
?>