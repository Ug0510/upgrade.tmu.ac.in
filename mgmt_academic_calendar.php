<?php include 'head.php'; ?>

<?php
 include 'sidebar/management_sidebar.php';
?>
	<div class="col-12 col-md-9 col-lg-9 mt-3 mt-sm-0 pt-5">
         <h1 class="tmu-text-primary tmu-page-heading"><span>Academic </span><span>Calendar</span></h1>

                <div class="container">  
                    <div class="row">
                        <object data="https://www.tmu.ac.in/pdf/pdf/TMIMT_AC_23_24.pdf" type="application/pdf" width="100%" height="1120px">
							<p>Your web browser doesn't have a PDF plugin.  Instead you can</p>
							<center><a type="button" class="btn btn-success btn-lg btn-rounded text-lg waves-effect waves-light" href="https://www.tmu.ac.in/pdf/pdf/TMIMT_AC_23_24.pdf" download="" data-uw-pdf-br="2" data-uw-pdf-doc=""><i class="fa fa-download fa-2x pr-2" aria-hidden="true"></i>Click here to download PDF file.</a></center>
						</object>
                    </div>
                </div>      

</div>
</div>
</div>
<?php include 'footer.php'; ?>    